...
AddPrefabPostInit("daywalker", function(boss)
    boss:DoTaskInTime(0.2, function(inst)
        ...
        if EventTimer.GetTimeFromRemoteCommand and not EventTimer.GetTimeFromServerMod["daywalkerspawner"] then
...
info = {
    ...
    tipsfn = function()
        if EventTimer.GetTimeFromServerMod["daywalkerspawner"] or not EventTimer.GetTimeFromRemoteCommand then
            local time = ThePlayer.HUD.WarningEventTimeData.daywalkerspawner_time
